...
### 1.1 二维码可以直接解析 IP 吗？
可以。二维码里直接放 `http://43.156.153.252:3101/`（或你反代后的 80/443 地址）即可打开页面。

### 1.2 Hoire 订阅回调可以先用 IP 吗？
可以先试：
- `APP_URL=http://43.156.153.252:3101`（直连容器映射端口）
- 或 `APP_URL=http://43.156.153.252`（如果你用 Nginx 80 反代到 3101）
...
HOIRE_ACCOUNT=你的账号
HOIRE_PASSWORD=你的密码
APP_URL=http://43.156.153.252:3101
PORT=3000
SITE_DEVICE_BINDINGS_JSON={"base-current":{"siteName":"当前基地","weatherId":11828,"insectId":2734,"cameraId":313793}}
VITE_AMAP_KEY=可选
...
你可以通过 `SITE_DEVICE_BINDINGS_JSON` 一次性配置多个基地绑定，后端会优先读取该变量：

```env
SITE_DEVICE_BINDINGS_JSON={"base-current":{"siteName":"当前基地","weatherId":11828,"insectId":2734,"cameraId":313793},"base-b":{"siteName":"B基地","weatherId":10001,"insectId":10002,"cameraId":10003}}
```

二维码可直接使用：

- `http://43.156.153.252:3101/?site=base-current`
- `http://43.156.153.252:3101/?site=base-b`

如果 `site` 不存在，会自动回退到默认基地 `base-current`。
...
- `http://43.156.153.252:3101/api/iot/ping`

或命令行：

```bash
curl http://43.156.153.252:3101/api/iot/ping
```
...
```json
{
  ...
  "callbackUrl": "http://43.156.153.252:3101/api/iot/receive"
}
```

## 4.2 在前端触发订阅

1. 打开：`http://43.156.153.252:3101/`
2. 点击“订阅所选设备”
3. 点击“查看订阅详情”
...
```bash
curl http://43.156.153.252:3101/api/iot/latest
```
...
```bash
ss -lntp | grep -E ':3101|:80|:443'
```

如 3101 被占用，改 `docker-compose.yml` 为其他宿主端口（如 `3201:3000`），并同步更新 `APP_URL`。
...
1. 开 3101 端口。  
2. `git clone` 代码。  
3. 配 `.env`（`APP_URL=http://43.156.153.252:3101`）。  
